...
import ConfirmDeleteModal, { ConfirmDeleteModalHandle } from "@/components/share/confirm-delete-modal";
...
export default function AdminHospitalPage() {
  ...
  const loadHospital = async (searchInput?: string) => {
    ...
    try {
      const rs = await http.get<any | null>(`/admin-hospital/hospitals${queryString ? `?${queryString}` : ""}`)
  ...
  const deleteHospital = async (id: number) => {
    ...
    setLoadingHospital(true)
    try {
      const rs = await http.delete<any | null>(`/admin-hospital/hospital/${id}`)
      if (rs) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold tracking-tight">Quản lý cơ sở y tế</h1>
      ...
      <div className="bg-white rounded-lg shadow-md p-4 mb-8">
        ...
        <div className="flex justify-end mt-4">
          <Link href={"/admin/facilities/new"}>
            <Button className="bg-teal-600 hover:bg-teal-700">Thêm cơ sở y tế</Button>
          </Link>
        </div>
      </div>
      <div className="rounded-md border">
                  ...
                  <TableRow key={hospital.id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <Button
                          onClick={() => { router.push(`/admin/facilities/${hospital.id}/edit`) }}
                          variant="outline"
                          size="icon"
        ...
        <div className="flex items-center justify-between px-4 py-2">
          <div className="text-sm text-muted-foreground">
            Hiển thị {(page - 1) * 5 + 1}-{Math.min(page * 5, total)} của {total} cơ sở y tế
          </div>
          <Pagination>
            <PaginationContent>
              <PaginationItem>
