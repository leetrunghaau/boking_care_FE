...
export default function Dashboard() {
  ...
  const [upcomingAppointments] = useState([
    { id: 1, patient: "Nguyễn Văn A", time: "09:00 - 09:30", service: "Khám da liễu" },
    { id: 2, patient: "Trần Thị B", time: "10:00 - 10:30", service: "Khám tổng quát" },
  ])
  ...
  return (
    <div className="w-11/12 mx-auto my-12 space-y-16">
      ...
      {/* Tổng quan nhanh */}
      <section>
        <h1 className="text-3xl font-bold text-slate-800 mb-6">Tổng quan công việc</h1>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          ...
          <div className="p-4 bg-green-50 rounded-lg text-center shadow">
            ...
            <p className="text-2xl font-bold text-green-600">{appointmentsThisWeek}</p>
          </div>
          <div className="p-4 bg-yellow-50 rounded-lg text-center shadow">
            ...
            <p className="text-2xl font-bold text-yellow-600">{earnings.toLocaleString()}</p>
          </div>
      ...
      <section>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          <Card className="shadow-sm border-teal-600 border-l-4 ">
            <CardHeader>
              <CardTitle className="text-slate-800">🗓️ Lịch khám sắp tới</CardTitle>
            </CardHeader>
            <CardContent className="p-4 space-y-4">
              {upcomingAppointments.length === 0 && (
                <p className="text-sm text-muted-foreground">Không có lịch khám sắp tới.</p>
              )}
              {upcomingAppointments.map((appt) => (
                <div key={appt.id} className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    <p className="text-sm font-medium text-slate-600">{appt.time}</p>
                    <Button size="sm" className="mt-1 bg-teal-600 text-white hover:bg-teal-700">
                      Xem hồ sơ
                    </Button>
                  </div>
                </div>
          ...
          <Card className="shadow-sm border-yellow-500 border-l-4 ">
            <CardHeader>
              <CardTitle className="text-slate-800">🔔 Thông báo hệ thống</CardTitle>
            </CardHeader>
          ...
          <Card className="shadow-sm border-teal-600 border-l-4 ">
            <CardHeader>
              <CardTitle className="text-slate-800">📊 Lượt khám theo tuần</CardTitle>
            </CardHeader>
