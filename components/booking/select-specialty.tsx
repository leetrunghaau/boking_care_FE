...
export default function SelectSpecialty({
  ...
  useEffect(() => {
    const fetchSpecialties = async () => {
      setLoading(true);
      try {
        const resSpecialty = await http.get<any[]>(`/booking/specialties`);
    ...
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600">
          Chọn triệu chứng hoặc chuyên khoa phù hợp để được tư vấn tốt nhất
        </p>
      </div>
      ...
      <div>
        <Input
          type="text"
          placeholder="Các triệu chứng bệnh của bạn"
          value={trieuChung}
      ...
      {filteredSpecialties.length === 0 && (
        <div className="text-center py-8">
          <p className="text-gray-500">
            Không tìm thấy chuyên khoa phù hợp. Vui lòng thử từ khóa khác.
          </p>
        </div>
      )}
    </div>
