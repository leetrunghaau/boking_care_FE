...
export default function DoctorRating({ slug }: Pops) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const res = await http.get<any>(`/doctor-site/doctor/${slug}/ratings/${reviewsIndex}`)
        setReviews(res);
      } catch (err) {
        console.error("Failed to fetch doctors:", err);
      } finally {
          ...
          <div className="border p-4 rounded-lg">
            ...
            <div className="flex items-center gap-1 mb-4">
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <Star
                    className={`h-6 w-6 ${star <= (hoveredRating || userRating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`}
                  />
                </button>
              ))}
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-1 my-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star
                      key={star}
                      className={`h-4 w-4 ${star <= Number(item.rating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`}
                    />
                  ))}
                </div>
