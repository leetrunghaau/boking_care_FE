...
export function ScheduleSettings() {
    ...
    const handleSave = async () => {
        ...
        try {
            const res = await http.post<any | null>(`/admin-doctor/doctor/${id}/schedule`, settings)
            if (res) {
                setSettings(res)
            }
    ...
    const handleLoad = async () => {
        ...
        setError(null)
        try {
            const res = await http.get<any>(`/admin-doctor/doctor/${id}/schedule`)
            ...
            <Card className="w-full max-w-[900px] mx-auto">
                <CardHeader className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                    <div>
                        ...
                        <CardDescription>Thiết lập mặc định cho lịch làm việc cho bác sĩ</CardDescription>
                    </div>
