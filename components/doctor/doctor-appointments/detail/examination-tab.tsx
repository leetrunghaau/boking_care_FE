...
export default function ExaminationTab({ bookingId }: Pops) {
    ...
    const [examination, setExamination] = useState<any>({
        ...
        height: "",
        folowUp: false,
        folowUpDate: moment().format('YYYY-MM-DD'),
        folowUpTime: "",
        folowUpNote: ""
 ...
 try {
    ...
    useEffect(() => {
        ...
        const fetchPatient = async () => {
            try {
                setLoading(true)
                const res = await http.get<any>(`/doctor-appointment/my-schedule/${examination.folowUpDate}`);
                setSchedule(res);
            } catch (err) {
                console.error("Failed to fetch appointment detail:", err);
            } finally {
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <Button onClick={hanhdelSaveExamination} className="bg-teal-600 hover:bg-teal-700" >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <Input
                                ...
                                onChange={(e) => handleChange("folowUpDate", e.target.value)}
                            />
                        </div>
                        <div>
                            <Select value={examination.folowUpTime} onValueChange={(e) => handleChange("folowUpTime", e)}>
                                <SelectTrigger >
                                    <SelectValue placeholder="Chọn giờ" />
                                </SelectTrigger>
                                <SelectContent>
                                    {schedule.map((item: any) => (<SelectItem key={item.start} value={item.start}>{item.start}</SelectItem>))}
                                </SelectContent>
                            </Select>
                        </div>
                    </div>
